...
let person = [
  { name: "KG", age: 12, votingStatus: false },
  { name: "DA", age: 78, votingStatus: false },
  { name: "MG", age: 45, votingStatus: false },
  { name: "MG", age: 11, votingStatus: false },
  { name: "Nk", age: 4, votingStatus: false },
];
//Given Array of object
//Global

router.post("/get", function (req, res) {
  //Create query param and req handler
  let queryParam = req.query.votingAge;
  //User request assigned qureyParam
  const result = person.filter((pers) =>
    pers.age > queryParam
      ? (pers.votingStatus = true)
      : (pers.votingStatus = false)
  );
  //Filtering persons from their age using filter (i.e higher Order Function)
  //changing their votingstatus
  console.log(result);
